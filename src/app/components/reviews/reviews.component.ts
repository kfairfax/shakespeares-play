...
export class ReviewsComponent implements OnInit {
  ...
  ngOnInit() {
    this.reviewsService.getReviews().subscribe((result: Review[]) => {
      this.reviews = result; //get an array of review objects
  ...
  onReviewClick(id) {
    this.router.navigate(['/reviews/' + id]); //naviagte by url using review's id as param
  }
  ...
  private iterator() { //provides the subarray of the original data array that is being shown at any given time based on user's paginator selection
